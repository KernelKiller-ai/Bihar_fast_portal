...
def is_official_https_url(url: Optional[str]) -> bool:
    ...
    try:
        ...
        return (
            ...
            and any(hostname == domain or hostname.endswith("." + domain) or hostname.endswith(".gov.in") or hostname.endswith(".nic.in") for domain in OFFICIAL_ALLOWED_DOMAINS)
        )
    except Exception:
        return False
